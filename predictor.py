...
def predict_eod(symbol):
    try:
        ...
        # Very safe feature list
        features = ['Open', 'High', 'Low', 'Close', 'Volume', 'EMA9', 'EMA21', 'RSI', 'MACD']
        
        X = df[features].copy()
        y = df['Close'].shift(-1)
        
        model = XGBRegressor(n_estimators=80, learning_rate=0.1, max_depth=4, random_state=42)
        model.fit(X.iloc[:-1], y.iloc[:-1])
        
        current = round(float(df['Close'].iloc[-1]), 2)
        predicted = round(float(model.predict(X.iloc[-1:].values)[0]), 2)
        
        upside = ((predicted - current) / current) * 100
        
        print(f"   ✅ Success → Current: ₹{current} | Predicted: ₹{predicted} | Upside: {upside:.2f}%")
        return current, predicted
        
    except Exception as e:
        print(f"   ❌ Error for {symbol}: {str(e)[:90]}")
        return None, None
